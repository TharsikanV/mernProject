...
import { productRequest, productFail, productSuccess, createReviewRequest, createReviewSuccess, createReviewFail, newProductRequest } from '../slices/productSlice';
...
export const createNewProduct=productData=>async(dispatch)=>{
    try{
        ...
        const {data}=await axios.post(`/api/v1/admin/product/new`,pr);
        dispatch(adminProductsSuccess(data))
    }catch(error){
        //handle error
        dispatch(adminProductsFail(error.response.data.message))
