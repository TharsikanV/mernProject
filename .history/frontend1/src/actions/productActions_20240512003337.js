...
import { productRequest, productFail, productSuccess, createReviewRequest, createReviewSuccess, createReviewFail, newProductRequest, newProductSuccess, newProductFail } from '../slices/productSlice';
...
export const deleProduct=productData=>async(dispatch)=>{
    try{
        dispatch(newProductRequest())//action creater ah ulla koduthiddam
        const {data}=await axios.post(`/api/v1/admin/product/new`,productData);
        dispatch(newProductSuccess(data))
