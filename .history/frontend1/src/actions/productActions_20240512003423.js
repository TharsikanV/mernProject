...
import { productRequest, productFail, productSuccess, createReviewRequest, createReviewSuccess, createReviewFail, newProductRequest, newProductSuccess, newProductFail, deleteProductRequest } from '../slices/productSlice';
...
export const deleteProduct=id=>async(dispatch)=>{
    try{
        ...
        const {data}=await axios.delete(`/api/v1/admin/product/new`,productData);
        dispatch(newProductSuccess(data))
    }catch(error){
        //handle error
        dispatch(newProductFail(error.response.data.message))
