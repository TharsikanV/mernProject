...
export default function ProductDetail() {
    const { loading, product,isReviewSubmitted,er} = useSelector((state) => state.productState);
    ...
    }, [dispatch, id,isReviewSubmitted])//eppa elaam isReviewSubmitted value maatha paditho getProduct(id) call pannapadum

    return (
        <Fragment>
            {loading ? <Loader /> ://ithu important eaanna component run aaka unnam product ellam undefined thaan
                <Fragment>
                    <MetaData title={product.name} />
                    <div className="row f-flex justify-content-around">
                        <div className="col-12 col-lg-5 img-fluid" id="product_image">
                            <Carousel pause='hover'>{/**mouse varum pothu image pause aakidanum */}
                                {product.images && product.images.map(image =>
                                    <Carousel.Item key={image._id}>{/**ovvoru image tag kkum thanithuvamaana key kodukkanum */}
                                            ...
                                            <ul className="stars" >
                                                {
                                                    [1,2,3,4,5].map(star=>(
                                                        ...
                                                        onMouseOver={(e)=>e.target.classList.add('yellow')}//yellow endra class ah add panniddam (ithu css class)
