...
const getProducts=catchAsyncError(async (req,res,next)=>{
    ...
    const filteredProductsCount=await buildQuery().query.countDocuments({});//filter seyyappadd product ooda count ah konduvarum
...
const newProduct=catchAsyncError(async(req,res,next)=>{
  ...
  if(req.files.length>0){//nama upload pannira file ah ipd thaan edukkalaam
...
const updateProduct=catchAsyncError(async(req,res,next)=>{
  ...
  if(req.body.imagesCleared==='false'){
    //frontendla earkanave iruntha images ah clear pannirathukkaana option ah koduppam
    //apd clear panna imagesCleared==='true' aakidum
...
const createReview=catchAsyncError(async(req,res,next)=>{
    ...
    const isReviewed=product.reviews.find(review=>{//array endrathaala find() use pannalaam
  ...
  product.reviews.forEach(review => {
      ...
      console.log(      totalRating += review.rating;
      );
  });
